...
def compute_daily_stats(rows: list) -> list:
    """Group readings into 24-hour day buckets relative to launch and compute per-day stats."""
    valid = []
    for r in rows:
        ...
        valid.append({
            "v": v,
            "l": safe_int(r.get("like_count")) or 0,
            "c": safe_int(r.get("comment_count")) or 0,
            "t": t,
        })
    ...
    by_day = {}
    for r in valid:
        hours_since = (r["t"] - launch).total_seconds() / 3600
        day_idx = int(hours_since // 24)
    ...
    for d in range(max_day + 1):
        ...
        if d == 0:
            ...
            prev_day_readings = by_day.get(d - 1, [])
            if prev_day_readings:
                start_v = prev_day_readings[-1]["v"]
                start_l = prev_day_readings[-1]["l"]
                start_c = prev_day_readings[-1]["c"]
                start_t = prev_day_readings[-1]["t"]
            else:
                start_v = readings[0]["v"]
                start_l = readings[0]["l"]
                start_c = readings[0]["c"]
        ...
        views_gained = end_v - start_v
        hours_covered = max((end_t - start_t).total_seconds() / 3600, 0.01)
        avg_per_hour = round(views_gained / hours_covered) if hours_covered > 0 else 0

        if end_v > 0:
            engagement = round(((end_l + end_c) / end_v) * 100, 2)
        else:
            engagement = None
...
def build_html(rows: list, meta: dict) -> str:
    ...
    for i, d in enumerate(daily):
        partial_badge = '<span class="daily-partial">In Progress</span>' if d["is_partial"] else ''
        trend_html = ''
        if i > 0 and d["engagement_rate"] is not None and daily[i-1]["engagement_rate"] is not None:
            delta = d["engagement_rate"] - daily[i-1]["engagement_rate"]
            if delta < -0.01:
                trend_html = f'<div class="daily-metric-trend down">▼ {abs(delta):.2f}pp vs Day {d["day"]-1}</div>'
            elif delta > 0.01:
                trend_html = f'<div class="daily-metric-trend up">▲ {delta:.2f}pp vs Day {d["day"]-1}</div>'
            else:
                trend_html = f'<div class="daily-metric-trend">flat vs Day {d["day"]-1}</div>'

        eng_str = f"{d['engagement_rate']}%" if d["engagement_rate"] is not None else "—"
        daily_cells.append(f'''
          <div class="daily-col">
            <div class="daily-col-head">
              <span class="daily-col-num">Day {d["day"]}</span>
              {partial_badge}
            </div>
            <div class="daily-metric">
              <span class="daily-metric-label">Views Added</span>
              <span class="daily-metric-value">+{d["views_gained"]:,}</span>
            </div>
            <div class="daily-metric">
              <span class="daily-metric-label">Avg / Hour</span>
              <span class="daily-metric-value">{d["avg_per_hour"]:,}</span>
            </div>
            <div class="daily-metric">
              <span class="daily-metric-label">Engagement Rate</span>
              <span class="daily-metric-value engagement">{eng_str}</span>
              {trend_html}
            </div>
          </div>
        ''')
    daily_html = "\n".join(daily_cells) if daily_cells else '<div class="daily-col"><div class="daily-metric-label">Not enough data yet</div></div>'
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .daily-metric-trend.down {{ color: var(--orange); }}
  .daily-metric-trend.up {{ color: var(--positive); }}
...
<div class="container">
  ...
  <div class="trailer-card">
    <a class="trailer-thumb" href="{VIDEO_URL}" target="_blank" rel="noopener">
      <span class="live-badge"><span class="live-badge-dot"></span>Tracking Complete</span>
  ...
  <div class="daily-card">
    <div class="daily-head">
      <span class="daily-title">Daily Breakdown</span>
      <span class="daily-subtitle">View accumulation and engagement rate by day since launch</span>
    </div>
